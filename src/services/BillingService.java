...
public class BillingService {
    ...
    public Bill generateBill(String customerId, String customerMobile, String customerName, int customerAge, String customerLocation,
        ...
        // Use a single connection for the entire billing transaction
        Connection conn = null;
        try {
            conn = DatabaseConfig.getConnection();
            conn.setAutoCommit(false);

            // 1. Resolve or register/update Customer (on shared connection)
            ...
            if (customerId != null && !customerId.trim().isEmpty()) {
                ...
                if (customer != null) {
                    // Check if mobile number is changed and if it collides with another customer
            ...
            for (PurchaseItemRequest req : itemsRequest) {
                // Fetch product on shared connection
                ...
                // Deduct inventory on shared connection
                ...
                // GST-inclusive calculations (PRP is inclusive of GST)
                ...
                // Final amount is exactly PRP * Qty (GST is included)
                BigDecimal finalAmount = prp.multiply(qty).setScale(2, RoundingMode.HALF_UP);
                
                // Reverse-calculate taxable value and GST components
                // taxableValue = finalAmount / (1 + (gstPct / 100))
                ...
                // Total GST = finalAmount - taxableValue
                ...
                // Discount is the difference between MRP total and final PRP total
            ...
            // Save bill header FIRST (bill_items FK depends on bills.bill_id)
    ...
    // --- Private helpers that operate on a shared connection ---
